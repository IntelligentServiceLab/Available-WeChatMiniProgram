			...
			<view class="demo-add" v-for="item in listArr">
				...
				<uni-col :span="12">
					<view class="demo-uni-col" style="padding-left: 20rpx;">
						...
						<view v-if="item.response == '1'" class="friend-text">
							{{item.reason}}
							{{item.location}}
						</view>

						<view v-if="item.response == '1'" class="friend-text" :class="[item.fontColor == '1' ? 'fontRed' : '',item.fontColor=='2' ? 'fontGray' : '']">
				...
				<uni-col :span="4">
					<view class="demo-uni-col right-bottom" >
							...
							<view v-if="item.response == '1'" class="outside right">
								...
								<text class="friend-text" style="margin-top: 2rpx;" >已订阅 </text>
							</view>
					...
					<!-- 		<uni-section v-if="item.response == '2'" title="对话框示例" type="line" class="outside right">
									...
									<text  @click="dialogToggle(item.rejectReason)" class="friend-text" style="margin-top: 2rpx;" >未订阅 </text>
							</uni-section> -->
							
							<view v-if="item.response == '0'" class="outside right">
								<image class="dialog" :src="url[2].urlNull" mode=""></image>
								<text class="friend-text" style="margin-top: 2rpx;" >未订阅 </text>
							</view>
...
<script>
	...
	export default {
		...
		methods: {
			getList() {
			    uni.request({
					url: 'http://127.0.0.1:4523/m1/3371748-0-default/friendApoint',
					success: (res) => {
